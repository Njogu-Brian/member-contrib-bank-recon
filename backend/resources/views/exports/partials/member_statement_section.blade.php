@php
    ...
    $summaryData = $summary ?? [];
    $contributionStatus = $summaryData['contribution_status_label'] ?? ($summaryData['contribution_status'] ?? 'Unknown');
    ...
    $difference = $summaryData['difference'] ?? ($totalContributions - $expectedContributions);
    $openingBalance = (float) ($summaryData['opening_balance'] ?? 0);
    $closingBalance = (float) ($summaryData['closing_balance'] ?? $openingBalance);
...
<div class="statement {{ ($pageBreak ?? false) ? 'page-break' : '' }}">
    ...
    <div class="note">
        Note: Credits are contributions received. Debits are invoices and approved expenses. Balance is the running ledger.
    </div>
    ...
    <table class="transactions">
        <thead>
            <tr>
                ...
                <th style="width: 14%;">Debit (KES)</th>
                <th style="width: 14%;">Credit (KES)</th>
                <th style="width: 16%;">Balance (KES)</th>
            </tr>
        </thead>
        <tbody>
            ...
            @forelse ($sortedEntries as $entry)
                @php
                    ...
                    if ($debit == 0.0 && $credit == 0.0 && isset($entry['amount'])) {
                    ...
                    $runningBalance = $entry['running_balance'] ?? (($openingBalance ?? 0) + $credit - $debit);
                    $totalDebits += $debit;
                    $totalCredits += $credit;
                    $dateDisplay = empty($entry['date']) ? '-' : \Carbon\Carbon::parse($entry['date'])->format('M d, Y');
                    $typeLabel = ucwords(str_replace('_', ' ', $entry['type'] ?? ''));
                @endphp
                <tr>
                    <td>{{ $dateDisplay }}</td>
            ...
            @empty
                <tr>
                    <td colspan="6" class="empty-row">No statement activity for the selected period.</td>
                </tr>
            @endforelse
        </tbody>
        <tfoot>
    ...
    <div class="summary-section" style="margin-top: 30px; padding: 20px; background-color: #f8f9fa; border: 2px solid #dee2e6; border-radius: 8px;">
        ...
        <table style="width: 100%; border-collapse: collapse;">
            <tr>
                <td style="padding: 8px 0; font-size: 14px;">Opening Balance:</td>
                <td style="padding: 8px 0; text-align: right; font-size: 14px;">KES {{ number_format($openingBalance, 2) }}</td>
            </tr>
            <tr>
            ...
            <tr>
                <td style="padding: 8px 0; font-size: 14px;"><strong>Closing Balance:</strong></td>
                <td style="padding: 8px 0; text-align: right; font-size: 14px; font-weight: bold;">KES {{ number_format($closingBalance, 2) }}</td>
            </tr>
    ...
    <div class="footer-text">
        ...
        @if(!empty($contactPhone))
            For clarification, contact <strong>{{ $contactPhone }}</strong>.
        @endif
    </div>
</div>
