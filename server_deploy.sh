#!/bin/bash
# Production Deployment Script - Run this on the server
...
cd /home/royalce1/laravel-app/evimeria || {
...
if [ -f dist.zip ]; then
    ...
    if [ -d dist_temp/dist ]; then
        PUBLIC_DIR="/home/royalce1/public_html/evimeria.breysomsolutions.co.ke"
    ...
    echo "  ⚠ WARNING: dist.zip not found (upload it manually if needed)"
fi
echo ""
...
echo "  - Migrations run (update only)"
echo "  - Caches cleared"
